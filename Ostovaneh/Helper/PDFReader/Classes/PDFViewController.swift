...
public final class PDFViewController: UIViewController {
    ...
    public var currentPageIndex: Int = 0 {
        willSet {
            if currentPageIndex < newValue {
                delegate?.PDFPage(moveTo: newValue, forward: true)
            } else {
                delegate?.PDFPage(moveTo: newValue, forward: false)
            }
        }
    }
    ...
    private var isThumbnailsEnabled = false {
        didSet {
    ...
    public func movePDFViewPage(to index: Int) {
        didSelectIndexPath(IndexPath(item: index, section: 0))
    }
}
